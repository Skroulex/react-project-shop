...
const CartContextProvider = ({ children }) => {
    ...
    function addToCart(product) {
        ...
        const isProductInCart = cart.products.some((item) => item.item.id === newProduct.item.id)
        console.log('tovar v korzine', isProductInCart)
        if (isProductInCart){
    ...
    return (
        <cartContext.Provider
            value={{ cart: state.cart, count: state.count, getCart, addToCart,deleteFromCart,changeCount }}>
            {children}
        </cartContext.Provider>
    );
};
